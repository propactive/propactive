...
#%    tag, a minor incremented version will be returned. (i.e. "2.0.0"
#%    => "2.0.1")
#%
...
function version_deriver() {
  ...
  if [ "$CURRENT_BRANCH" = "main" ]; then
    ...
    local LATEST_VERSIONED_MERGE_COMMIT=$(git log --merges -n 1 | grep request | grep -oh -P "(?<=\/)\d+\.\d+\.\d+$")

    if [ -z "$LATEST_TAG" ] && [ -z "$LATEST_VERSIONED_MERGE_COMMIT" ]; then
      VERSION="1.0.0"
    elif [ "$LATEST_TAG_COMMIT_ID" = "$(git rev-parse HEAD)" ]; then
      VERSION=$LATEST_TAG
    elif [ "$LATEST_TAG" = "$LATEST_VERSIONED_MERGE_COMMIT" ] || [ -z "$LATEST_VERSIONED_MERGE_COMMIT" ]; then
